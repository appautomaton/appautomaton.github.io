...
import {artworks,artworkManifest,assignArtwork} from './artwork.mjs'
...
const registry=JSON.parse(await readFile(join(ROOT,'src/data/catalog-state.json'),'utf8'))
...
for(const asset of artworks) {
 ...
 artAssets[asset.key]='assets/objects/'+file
 const variants=[]
 ...
 artSources[asset.key]=[...variants,`${artAssets[asset.key]} 720w`].join(', ')
}
const assigned=assignArtwork(groups.flatMap(g=>g.items.map(p=>({...p,group:g.key}))),registry)
const projects=[]
for(const group of groups) for(const p of group.items) {
 ...
 const asset=assigned.get(project.repo)
 project.art=artAssets[asset.key]
 project.artSrcset=artSources[asset.key]
 project.artKey=asset.key
 project.artAlt=asset.alt
 projects.push(project)
}
for(const group of groups) {
 group.items=projects.filter(p=>p.group===group.key)
 group.art=artAssets[artworkManifest.covers[group.key]]
 group.artSrcset=artSources[artworkManifest.covers[group.key]]
...
const head=(title,desc,canonical=rootUrl)=>`<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(title)}</title><meta name="description" content="${e(desc)}"><meta name="robots" content="index, follow, max-image-preview:large"><link rel="canonical" href="${e(canonical)}"><meta name="theme-color" content="#f1efe9"><meta property="og:type" content="website"><meta property="og:site_name" content="App Automaton"><meta property="og:title" content="${e(title)}"><meta property="og:description" content="${e(desc)}"><meta property="og:url" content="${e(canonical)}"><meta property="og:image" content="${rootUrl}assets/${socialName}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="App Automaton. Tools for thought, built for real work."><meta property="og:image:type" content="image/png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${e(title)}"><meta name="twitter:description" content="${e(desc)}"><meta name="twitter:image" content="${rootUrl}assets/${socialName}"><meta name="twitter:image:alt" content="App Automaton. Tools for thought, built for real work."><link rel="icon" href="assets/mark.svg" type="image/svg+xml"><link rel="apple-touch-icon" href="assets/${touchName}" sizes="180x180"><link rel="preload" href="assets/fonts/UncutSans-Regular.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="assets/fonts/UncutSans-Medium.woff2" as="font" type="font/woff2" crossorigin><script>(()=>{const r=document.documentElement;let t,m;try{t=localStorage.getItem('aa-mode');m=localStorage.getItem('aa-motion')}catch{}r.dataset.theme=t==='light'?'day':t==='dark'?'night':matchMedia('(prefers-color-scheme: dark)').matches?'night':'day';r.dataset.motion=m==='off'||matchMedia('(prefers-reduced-motion: reduce)').matches?'off':'on'})()</script><link rel="stylesheet" href="assets/${style}">`
// One named primary link combines each project's artwork, heading, and action.
// The repository link remains separate when the project also has a website.
const card=p=>`<article class="project" id="project-${e(p.repo)}" data-group="${p.group}" data-search="${e([p.repo,p.description,p.groupName,...p.chips,...p.alsoKnownAs].join(' ').toLowerCase())}"><a class="project-primary" href="${e(p.site??p.source)}"><div class="project-visual"><span class="project-number" aria-hidden="true">${p.tag}</span><div class="project-image"><img src="${p.art}" srcset="${p.artSrcset}" sizes="(max-width:759px) 30vw, 18vw" alt="${e(p.artAlt)}" width="720" height="720" loading="lazy" decoding="async"><span class="image-arrow" aria-hidden="true">${icon('arrow')}</span></div></div><h3>${e(p.repo).replaceAll('-','-<wbr>')}</h3><span class="project-action">${p.site?'View project':'Explore the source'}${icon('arrow')}</span></a><div class="project-category"><span>${e(p.groupName)}</span><span>${p.site?'Project website':'Source available'}</span></div><p class="project-description">${e(p.description)}</p>${p.site?`<a class="project-source" href="${e(p.source)}">GitHub<span class="sr-only"> source for ${e(p.repo)}</span>${icon('arrow')}</a>`:''}</article>`

const featured=artworkManifest.featured.map(name=>projects.find(p=>p.repo===name)).filter(Boolean)
for(const p of projects)if(featured.length<6&&!featured.includes(p))featured.push(p)
const figures=featured.map((p,i)=>`<a class="floating-object object-${i+1}" href="${e(p.site??p.source)}" aria-label="Explore ${e(p.repo)}" style="--float-delay:-${i*2.3}s;--float-duration:${12+i*1.7}s"><div class="object-drift"><img src="${p.art}" srcset="${p.artSrcset}" sizes="(max-width:759px) 38vw, 23vw"${i===3?' fetchpriority="high"':''} alt="" width="720" height="720" decoding="async"><span class="object-label">${e(p.repo)} ${icon('arrow')}</span></div></a>`).join('')
...
<footer data-reveal><div class="footer-top"><a class="brand" href="#top">${logo()}<span>App<br>Automaton</span></a><p>Independent thinking.<br>Shared possibilities.</p><div><a href="https://github.com/appautomaton">GitHub organization ${icon('arrow')}</a><a href="https://huggingface.co/appautomaton">Hugging Face ${icon('arrow')}</a><a href="llms.txt">Text edition ${icon('arrow')}</a><a href="colophon.html">Art & colophon ${icon('arrow')}</a><button type="button" class="share-workshop" hidden><span>Share this page</span>${icon('share')}</button><span class="share-status sr-only" role="status"></span><input class="share-url" aria-label="Link to share" readonly hidden value="https://appautomaton.com/"></div></div><div class="footer-word" aria-hidden="true"><span>Keep making.</span></div><div class="footer-bottom"><span>An <a href="https://www.appcubic.com/">AppCubic</a> workshop.</span><a href="https://www.renocrypt.com/">Research & writing at RenoCrypt ${icon('arrow')}</a><a href="#top">Back to the top ${icon('arrow')}</a></div></footer></body></html>`
...
await writeFile(join(OUT,'catalog.json'),JSON.stringify({organization:'appautomaton',url:rootUrl,catalogStateVersion:1,projects:projects.map(({art,artSrcset,artKey,artAlt,...p})=>p)},null,2)+'\n')
await writeFile(join(OUT,'catalog-state.json'),JSON.stringify(registry,null,2)+'\n')
const artCredits=artworkManifest.models.map(asset=>`<figure class="art-credit"><a href="${e(asset.url)}"><img src="${artAssets[asset.key]}" srcset="${artSources[asset.key]}" sizes="(max-width:759px) 42vw, 22vw" width="720" height="720" alt="${e(asset.alt)}" loading="lazy" decoding="async"><span>${e(asset.title)} ${icon('arrow')}</span></a><figcaption>${e(asset.authors.join(', '))}<br>CC0 / Poly Haven</figcaption></figure>`).join('')
await writeFile(join(OUT,'colophon.html'),`<!doctype html><html lang="en"><head>${head('Art & colophon · App Automaton','The public 3D models, original sculptures, and independent type behind the App Automaton workshop.',rootUrl+'colophon.html')}</head><body><main class="colophon-page"><a class="text-link" href="./">Back to App Automaton ${icon('arrow')}</a><h1>Objects with<br>character.</h1><h2>An open collection of forms.</h2><p>Optical instruments, paper, wood, stone, shells, and sound equipment give the projects different silhouettes and material character. These models come from <a href="https://polyhaven.com/models">Poly Haven</a> and are freely available under <a href="https://polyhaven.com/license">CC0</a>. They were selected and rendered for this website. Follow an object below to its original model and artist.</p><div class="art-credits">${artCredits}</div><h2>Original marks, a shared setting.</h2><p>The rounded-square aperture, earlier abstract sculptures, architectural scenes, and interface SVGs were authored for App Automaton. The model collection uses the CC0 <a href="https://polyhaven.com/a/studio_small_09">Studio Small 09</a> lighting environment by Sergej Majboroda. The browser receives finished, compressed images rather than a 3D engine.</p><h2>Type with a clear voice.</h2><p>Uncut Sans by Kasper Nordkvist is self-hosted under the SIL Open Font License. <a href="assets/fonts/UncutSans-LICENSE.txt">Read the license</a> or <a href="https://github.com/kaspernordkvist/uncut_sans">visit the original source</a>.</p><h2>A point of reference.</h2><p><a href="https://floema.com/en">Floema</a> inspired the spatial composition, generous type, and editorial rhythm. Its photographs, product images, fonts, and illustrations are not used here.</p><h2>Collected at the source.</h2><p>Project facts come from the public App Automaton GitHub organization. The page refreshes daily and on demand. Project websites, source repositories, structured data, and discovery files share the same catalog. Art assignments are curated separately, with spare objects available for new projects.</p><h2>Room for every reader.</h2><p>The complete catalog works without JavaScript. Motion is optional and follows device preferences. Every project link is a normal, followable HTML link. The catalog illustrations have concise image descriptions, while repeated previews and ambient backgrounds remain decorative.</p><a class="text-link" href="https://github.com/appautomaton/appautomaton.github.io">Read the website source ${icon('arrow')}</a></main></body></html>`)
